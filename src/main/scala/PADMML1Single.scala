...
//      s.t x_i - z = 0
abstract class PADMML1Single(val datx: Array[DenseMatrix[Double]],
                             val daty: Array[DenseVector[Double]]) {
    // Dimension of x
    private val dim_x = datx(0).cols
    // Number of partitions
    private val npart = datx.length
    // Penalty parameter
    ...
    def run() {
        ...
        loop.breakable {
            for(i <- 0 until max_iter) {
                ...
                // x step
                admm_x = datx.zip(daty).zip(admm_y).map(t => update_x(t._1._1, t._1._2, rho, admm_z - t._2 / rho))
                ...
                // y step
                val resid = admm_x.map(x => x - admm_z)
                resid_primal = math.sqrt(resid.map(x => square(norm(x))).sum)
                admm_y = admm_y.zip(resid).map(t => t._1 + rho * t._2)
